...
export function searchForLanguage(
  ...
  const prefixOnlyFuse = new Fuse(languages as ILanguage[], {
    ...baseFuseOptions,
    threshold: 0.2, // we can turn this up if we find it's prioritizing things that are not so close matches
  ...
  const locationIndependentFuse = new Fuse(languages as ILanguage[], {
    ...baseFuseOptions,
    threshold: 0.2,
  });
  const locationIndependentResults =
    locationIndependentFuse.search(queryString);
  ...
  const filteredLocationIndependentResults = locationIndependentResults.filter(
    (result) => !prefixOnlyResultsCodes.has(result.item.iso639_3_code)
  );
  return [...prefixOnlyResults, ...filteredLocationIndependentResults];
}
