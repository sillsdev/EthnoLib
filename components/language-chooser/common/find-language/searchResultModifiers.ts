...
export function filterOutSpecialCaseExcludedLanguages(
  ...
  return filterLanguageCodes(
    ((code) => !SPECIAL_CASE_EXCLUDED_ENTRY_CODES.has(code)) as (
      value: string
    ) => boolean,
    results
  );
...
export function filterOutNonLivingLanguages(results: ILanguage[]): ILanguage[] {
  return results.filter(
    (result) => result.languageType === LanguageType.Living
  );
}
...
export function defaultSearchResultModifier(
  ...
  modifiedResults = filterOutNonLivingLanguages(modifiedResults);
