...
describe("get shortest equivalent version of langtag", () => {
  ...
  it("should preserve private use subtags with representative languages", () => {
    expect(
...
describe("get maximal equivalent version of langtag", () => {
  ...
  it("should handle representative languages with language parameter", () => {
    expect(getMaximalLangtag("uzn", NORTHERN_UZBEK_LANGUAGE)).toContain(
      "-Latn-UZ"
    );
    expect(getMaximalLangtag("uzn-Latn", NORTHERN_UZBEK_LANGUAGE)).toContain(
      "-Latn-UZ"
    );
    expect(getMaximalLangtag("uzn-Cyrl", NORTHERN_UZBEK_LANGUAGE)).toContain(
      "-Cyrl-UZ"
    );

    // ar (Standard Arabic) defaults to Arab script
    expect(getMaximalLangtag("arb", STANDARD_ARABIC_LANGUAGE)).toContain(
      "-Arab-EG"
    );
...
describe("createTagFromOrthography", () => {
  ...
  // See macrolanguages.md. Especially special cases Akan (aka/ak) and Sanskrit (san/sa) are massaged by default in the
  // search result modifier (searchResultModifier.ts) and so are tested with the modifier instead of here
