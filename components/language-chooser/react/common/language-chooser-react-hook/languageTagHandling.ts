...
// TODO test
export function isValidBcp47Tag(targets: string | undefined): boolean {
  ...
  return !!targets && bcp47Regex.test(targets);
}
...
export function createTagFromOrthography(orthography: IOrthography): string {
  ...
  if (
    codeMatches(
      strippedOrthography.language?.iso639_3_code,
      CODE_FOR_MANUALLY_ENTERED_TAG
    )
  ) {
    // This is a custom langtag the user entered by hand
    return strippedOrthography.language?.languageSubtag || "";
  }
  return createTag({
...
export function parseLangtagFromLangChooser(
  ...
  if (!language) {
    console.log("langtag parsing found unexpected language", languageSubtag);
    return undefined;
  }
  ...
  if (regionSubtag && !region) {
    console.log("langtag parsing found unexpected region", regionSubtag);
    return undefined;
  }
  ...
  // if the parsed language has scripts and the parsed script doesn't match any of them, this must be a tag requiring manual entry
  if (
    language.scripts.length > 0 &&
    !language.scripts.some((s) => codeMatches(s.code, script?.code))
  ) {
    console.log(
      "langtag parsing found unexpected script",
      scriptSubtag,
      language.scripts
    );
    return undefined;
