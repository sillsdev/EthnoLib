...
describe("defaultRegionForLangTag", () => {
  it("should return the region for a language tag that already has a region", () => {
    expect(defaultRegionForLangTag("en-Latn-CN")?.name).toEqual("China");
    expect(defaultRegionForLangTag("en-CN-x-foobar")?.name).toEqual("China");
    expect(defaultRegionForLangTag("en-CN")?.name).toEqual("China");
  });
  it("should return the region for the closest maximal equivalent of the langauge tag", () => {
